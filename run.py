...
def run(dataset: ImageDataset, option: argparse.Namespace):
    ...
    if option.depthNet == 0:
        ...
        leresmodel.load_state_dict(strip_prefix_if_present(checkpoint["depth_model"], "module."), strict=True)
        del checkpoint
    ...
    for image_ind, images in enumerate(dataset):
        ...
        whole_image_optimal_size, patch_scale = calculate_processing_resolution(
            img, option.net_receptive_field_size, r_threshold_value, scale_threshold, whole_size_threshold
        )

        print("\t wholeImage being processed in :", whole_image_optimal_size)

        # Generate the base estimate using the double estimation.
        whole_estimate = double_estimate(
            img, option.net_receptive_field_size, whole_image_optimal_size, option.pix2pixsize, option.depthNet
        )
        if option.R0 or option.R20:
            ...
            if option.output_resolution == 1:
                midas.utils.write_depth(
                    path,
                    cv2.resize(
                        whole_estimate, (input_resolution[1], input_resolution[0]), interpolation=cv2.INTER_CUBIC
                    ),
                    bits=2,
        ...
        for patch_ind in range(len(imageandpatchs)):
            ...
            patch_estimation = double_estimate(
                patch_rgb, option.net_receptive_field_size, option.patch_netsize, option.pix2pixsize, option.depthNet
            )
            ...
            patch_estimation = cv2.resize(
                patch_estimation, (option.pix2pixsize, option.pix2pixsize), interpolation=cv2.INTER_CUBIC
            )

            patch_whole_estimate_base = cv2.resize(
                patch_whole_estimate_base, (option.pix2pixsize, option.pix2pixsize), interpolation=cv2.INTER_CUBIC
            )
        ...
        if option.output_resolution == 1:
            ...
            midas.utils.write_depth(
                path, imageandpatchs.estimation_updated_image, bits=2, colored=option.colorize_results
            )
...
def double_estimate(img: npt.NDArray, size1, size2, pix2pix_size: int, net_type):
...
def estimate_leres(image_array: npt.NDArray, leres_inference_size: int) -> npt.NDArray:
    ...
    ressized_rearranged_image_array = cv2.resize(
        rearranged_image_array_copy, (leres_inference_size, leres_inference_size)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", type=str, required=True, help="input files directory " "Images can be .png .jpg .tiff"
    )
    parser.add_argument(
        "--output_dir", type=str, required=True, help="result dir. result depth will be png." " vides are JMPG as avi"
    )
    parser.add_argument(
        "--savepatchs", type=int, default=0, required=False, help="Activate to save the patch estimations"
    )
    parser.add_argument(
        "--savewholeest", type=int, default=0, required=False, help="Activate to save the base estimations"
    )
    parser.add_argument(
        "--output_resolution",
        type=int,
